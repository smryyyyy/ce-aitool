...
- **本地模型支持**：配合 LM Studio / Ollama / vLLM，完全离线运行，无需联网
...
1. 将 `AITools` 文件夹整体复制到 Cheat Engine 的 Extensions 目录下：
   ```
   C:\Program Files\Cheat Engine 7.5\Extensions\AITools\
   ```
2. 启动 Cheat Engine，打开任意进程
3. 菜单栏 → **查看** → **AI Dialog**（或按快捷键）
...
检查：
- API URL 是否正确（LM Studio 默认 `http://localhost:1234`）
